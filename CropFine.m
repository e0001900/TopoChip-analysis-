...
% Outcome: finely cropped images will be stored in 'FineCropped' folder

clc; clear; close all

%% Variables to adjust
rotH = 0; % angle between top edge and horizontal line in deg, range:(-45,45], recomended:(-20 20]
...
thres = 0.5; % threshold for hough peaks
images = dir([dirIn f '*.' imtype]); % specs for original image

dirOut = [dirIn f 'FineCropped'];
mkdir(dirOut);

Cropped = false(length(images),1); % initialize manual cropping check
...
while ~all(Cropped) && thres>0
    parfor i = 1:length(images)
        ...
        P = houghpeaks(H,20,'Threshold',thres*max(H(:)));
...
%% Manual cropping
manualCropCheck = sum(~Cropped);
if manualCropCheck
    manualCropIm = {images(~Cropped).name}';
    ...
    switch choice
        case 'Yes'
            ...
            for i = 1:length(manualCropIm)
                ...
                Mask = createMask(h);
                tw = Tiff([dirOut f manualCropIm{i}],'w');
                writeTiffStackMask(Mask,tr,tw,tags)
                close(tr)
                close(tw)
                close
